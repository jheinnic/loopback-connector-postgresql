/*!
 * PostGIS connector for LoopBack
 */
...
var debug = require('debug')('loopback:connector:PostGIS');


/**
 *
 * Initialize the PostGIS connector against the given data source
 *
 * @param {DataSource} dataSource The loopback-datasource-juggler dataSource
 * @callback {Function} [callback] The callback function
 * @param {String|Error} err The error string or object
 * @header PostGIS.initialize(dataSource, [callback])
 */
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  dataSource.connector = new PostGIS(postgresql, dbSettings);
  dataSource.connector.dataSource = dataSource;

  definePostGISTypes(dataSource);
 ...
 */
function definePostGISTypes(dataSource) {
  ...
  defineType(function Point(data) {
  ...
  defineType(function PointZ(data) {
    if (!(this instanceof PointZ)) {
      return new PointZ(data);
    }

    console.log('PointX:', data, data.shell, data.shell.points);
  ...
  defineType(function Polygon(data) {
 ...
 * @property {String} hostname The host name or ip address of the PostGIS DB server
 * @property {Number} port The port number of the PostGIS DB Server
 ...
 * @constructor
 */
function PostGIS(postgresql, settings) {
  //this.name = 'PostGIS';
  //this._models = {};
...
require('util').inherits(PostGIS, SqlConnector);

PostGIS.prototype.debug = function () {
 ...
 */
PostGIS.prototype.connect = function (callback) {
  var self = this;
 ...
 */
PostGIS.prototype.executeSQL = function (sql, params, callback) {
  var self = this;
  var time = Date.now();
  var log = self.log;
 ...
 * @param {Object[]} data The result from the SQL
 */
PostGIS.prototype.query = function (sql, params, callback) {
 ...
 *
 */
PostGIS.prototype.count = function count(model, callback, filter) {
 ...
 * @param {String|Error} err The error string or object
 */
PostGIS.prototype.destroyAll = function destroyAll(model, where, callback) {
 ...
 * @private
 */
PostGIS.prototype._categorizeProperties = function(model, data) {
...
PostGIS.prototype.mapToDB = function (model, data) {
  var dbData = {};
  if (!data) {
    return dbData;
  }
  ...
  for (var p in data) {
    if(props[p]) {
      ...
      } else if(pType === 'Point' && data[p]) {
        // TODO Value to Object #1
        dbData[p] = vsprintf('SRID=4326;Point( %1$f %2$f )', [data[p].x, data[p].y]);
        // dbData[p] = 'SRID=4326;' + writer.write(data[p]);
      } else if(pType === 'PointZ' && data[p]) {
        // TODO Value to Object #2
        dbData[p] = vsprintf('SRID=4326;PointZ( %1$f %2$f %3$f )', [data[p].x, data[p].y, data[p].z]);
        // dbData[p] = vsprintf('SRID=4326;' + writer.write(data[p]);
      } else if(pType === 'Polygon' && data[p]) {
        // TODO Value to Object #3
        dbData[p] = 'SRID=4326;' + writer.write(data[p]);
      } else {
        dbData[p] = data[p];
      }
...
/**
 * Create the data model in PostGIS
 *
 * @param {String} model The model name
 ...
 */
PostGIS.prototype.create = function (model, data, callback) {
  var self = this;
...
// PostGIS 9.1 or later as it requres writable CTE
// See https://github.com/strongloop/loopback-connector-PostGIS/issues/27
/**
 ...
 */
/*
PostGIS.prototype.updateOrCreate = function (model, data, callback) {
  var self = this;
  data = self.mapToDB(model, data);
  var props = self._categorizeProperties(model, data);
...
/**
 * Save the model instance to PostGIS DB
 * @param {String} model The model name
 * @param {Object} data The model instance data
 * @callback {Function} [callback] The callback function
 * @param {String|Error} err The error string or object
 */
PostGIS.prototype.save = function (model, data, callback) {
  var self = this;
  data = self.mapToDB(model, data);
  var props = self._categorizeProperties(model, data);
...
PostGIS.prototype.update =
  PostGIS.prototype.updateAll = function (model, where, data, callback) {
 ...
 */
PostGIS.prototype.toFields = function (model, data, forCreate) {
  var self = this;
  var props = self._categorizeProperties(model, data);
...
function dateToPostGIS(val, dateOnly) {
  function fz(v) {
    return v < 10 ? '0' + v : v;
  }
 ...
 */
PostGIS.prototype.toDatabase = function (prop, val) {
  if (val === null || val === undefined) {
    // PostGIS complains with NULLs in not null columns
  ...
  if (val.constructor.name === 'Object') {
    if (prop.PostGIS && prop.PostGIS.dataType === 'json') {
  ...
  if (prop.type.name === 'Date' || prop.type.name === 'Timestamp') {
    ...
    /*
     ...
     return dateToPostGIS(val, prop.type.name === 'Date');
     */
  }

  // PostGIS support char(1) Y/N
  ...
  if (prop.type.name === 'GeoPoint' || prop.type.name === 'PGPoint') {
    if (val) {
  ...
  if (prop.type.name === 'Point') {
    if (val) {
      return "'SRID=4326;" + writer.write(val) + "'::geometry('point',4326)";
    } else {
      return 'NULL';
    }
  }

  if (prop.type.name === 'PointZ') {
    if (val) {
      return "'SRID=4326;" + writer.write(val) + "'::geometry('pointz',4326)";
    } else {
      return 'NULL';
    }
  }

  if (prop.type.name === 'Polygon') {
    if (val) {
      return "'SRID=4326;" + writer.write(val) + "'::geometry('polygon',4326)";
    } else {
      return 'NULL';
    }
  }
 ...
 */
PostGIS.prototype.fromDatabase = function (model, data) {
  if (!data) {
    return null;
  }
  var props = this._models[model].properties;
  ...
  for (var p in props) {
    ...
    } else if(prop && type === 'PointZ') {
      console.log('PointZ:', val);
      if (val !== null) {
        json[p] = reader.read(val);
      } else {
        json[p] = null;
      }
    } else if(prop && type === 'Point') {
    ...
    } else if (prop && type === 'GeoPoint' || type === 'PGPoint') {
 ...
 */
PostGIS.prototype.dbName = function (name) {
  if (!name) {
    return name;
  }
  // PostGIS default to lowercase names
  return name.toLowerCase();
};

/*!
 * Escape the name for PostGIS DB
 * @param {String} name The name
 * @returns {String} The escaped name
 */
PostGIS.prototype.escapeName = function (name) {
  if (!name) {
    return name;
  }
...
PostGIS.prototype.schemaName = function (model) {
  // Check if there is a 'schema' property for PostGIS
  var dbMeta = this._models[model].settings && this._models[model].settings.PostGIS;
  var schemaName = (dbMeta && (dbMeta.schema || dbMeta.schemaName))
    || this.settings.schema || 'public';
  return schemaName;
};

PostGIS.prototype.table = function (model) {
  // Check if there is a 'table' property for PostGIS
  var dbMeta = this._models[model].settings && this._models[model].settings.PostGIS;
  var tableName = (dbMeta && (dbMeta.table || dbMeta.tableName)) || model.toLowerCase();
  return tableName;
};

PostGIS.prototype.tableEscaped = function (model) {
 ...
 * @returns {String}
 */
PostGIS.prototype.getColumns = function (model, props) {
 ...
 */
PostGIS.prototype.all = function all(model, filter, callback) {
...
PostGIS.prototype.buildWhere = function (model, conds) {
...
PostGIS.prototype._buildWhere = function (model, conds) {
  if (!conds) {
    return '';
  }
  var self = this;
 ...
 * @returns {*}
 */
PostGIS.prototype.toFilter = function (model, filter) {
  var self = this;
  ...
  // http://docs.PostGIS.com/cd/B19306_01/server.102/b14200/functions137.htm#i86310
 ...
 *
 */
PostGIS.prototype.exists = function (model, id, callback) {
 ...
 */
PostGIS.prototype.find = function find(model, id, callback) {
  var sql = 'SELECT * FROM ' +
    this.tableEscaped(model);
  ...
  function decoratedCallback(err, data) {
    ...
    if (!err) {
      data.forEach(function (field) {
        field.type = mapPostGISDatatypes(field.type);
      });
    }
    cb(err, data);
  }
 ...
 * @param {String|Error} err The error string or object
 */
PostGIS.prototype.autoupdate = function(models, cb) {
  var self = this;
 ...
 * @param {Function} [cb] The callback function
 */
PostGIS.prototype.isActual = function(models, cb) {
  var self = this;
 ...
 * @param {Function} [cb] The callback function
 */
PostGIS.prototype.alterTable = function (model, actualFields, cb) {
  var self = this;
 ...
 * @returns {String}
 */
PostGIS.prototype.propertiesSQL = function (model) {
  var self = this;
  var sql = [];
 ...
 * @returns {*|string}
 */
PostGIS.prototype.propertySettingsSQL = function (model, propName) {
  var self = this;
 ...
 * @param {Function} [cb] The callback function
 */
PostGIS.prototype.dropTable = function (model, cb) {
  var self = this;
 ...
 * @param {Function} [cb] The callback function
 */
PostGIS.prototype.createTable = function (model, cb) {
  var self = this;
  var name = self.tableEscaped(model);

  // Please note IF NOT EXISTS is introduced in PostGIS v9.3
 ...
 * @param {Function} [cb] The callback function
 */
PostGIS.prototype.disconnect = function disconnect(cb) {
  if (this.pg) {
    if (this.settings.debug) {
...
PostGIS.prototype.ping = function(cb) {
 ...
 */
PostGIS.prototype.columnDbDefault = function(model, property) {
 ...
 */
PostGIS.prototype.columnDataType = function (model, property) {
  var columnMetadata = this.columnMetadata(model, property);
  ...
  switch (prop.type.name) {
    ...
    case 'PGPoint':
      return 'POINT';
    case 'Boolean':
      return 'BOOLEAN'; // PostGIS doesn't have built-in boolean
    case 'Point':       // PostGIS 2D (X,Y) Geometry Point
      return 'geometry(\'point\',4326)';
    case 'PointZ':      // PostGIS 3D (X,Y,Z) Geometry Point
      return 'geometry(\'pointz\',4326)';
    case 'Polygon':     // PostGIS Simple Polygon
      return 'geometry(\'polygon\',4326)';
  }
};

/*!
 * Map PostGIS data types to json types
 * @param {String} PostGISType
 * @param {Number} dataLength
 * @returns {String}
 */
function PostGISDataTypeToJSONType(PostGISType, dataLength) {
  var type = PostGISType.toUpperCase();
  switch (type) {
    case 'BOOLEAN':
      return 'Boolean';
    ...
    case 'geometry(\'polygon\',4326)':
      return 'Polygon';
    case 'geometry(\'pointz\',4326)':
      return 'PointZ';
    case 'geometry(\'point\',4326)':
      return 'Point';
...
function mapPostGISDatatypes(typeName) {
  return typeName;
}
